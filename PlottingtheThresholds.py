...
fig, ax = plt.subplots()
ax.plot(spike_samp, 0 * np.ones(spike_samp.size), '|')



gtOutput=AutoSortingExtractor(sortingPath)
#Now we will be plotting on the same plot the spike train of the ground truth

gtSpikes=gtOutput.get_unit_spike_train(unit_id=0)
gtSpikes2=gtOutput.get_unit_spike_train(unit_id=1)

ax.plot(gtSpikes, 0 * np.ones(gtSpikes.size), '|')
ax.plot(gtSpikes2,0*np.ones(gtSpikes2.size),'|')
plt.show()
